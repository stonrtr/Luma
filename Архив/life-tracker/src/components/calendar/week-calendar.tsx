...
export function WeekCalendar({ data, locale = "uk" }: { data: WeekData; locale?: string }) {
          ...
          <div className="grid border-t bg-muted/30" style={{ gridTemplateColumns: "56px repeat(7, minmax(96px, 1fr))" }}>
            ...
            {days.map((s, i) => (
              <div key={i} className="space-y-1 border-l p-2 text-[11px]">
                ...
                <div>
                  <span className="text-muted-foreground">{t(locale, "cal.tasksN")}</span>
                  <div className="font-medium">{s.summary.tasksDone} / {s.summary.tasksPlanned}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">{t(locale, "cal.factPlan")}</span>
                  <div className="font-medium">{formatMinutes(s.summary.actualMin, locale)} / {formatMinutes(s.summary.plannedMin, locale)}</div>
                </div>
              </div>
            ))}
          </div>
