  ...
  return (
    <Popover>
      <PopoverTrigger className="inline-flex items-center gap-1 rounded-md border border-dashed px-2 py-0.5 text-[11px] text-muted-foreground hover:bg-muted">
        <TagIcon className="size-3" /> {tr("tp.tags")}
      </PopoverTrigger>
      <PopoverContent className="w-64" align="start">
        <p className="mb-2 text-xs font-medium text-muted-foreground">{tr("task.tags")}</p>
        <div className="space-y-1">
          ...
          {allTags.length === 0 && <p className="text-xs text-muted-foreground">{tr("task.noTags")}</p>}
        </div>

        <div className="mt-3 border-t pt-3">
          <p className="mb-1.5 text-xs font-medium text-muted-foreground">{tr("task.newTag")}</p>
