      ...
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {canManage && !isSelf && status === "PENDING" && (
            <Button size="sm" variant="outline" onClick={() => { const c = window.prompt(t(locale, "plan.returnPrompt")) ?? undefined; start(async () => { const r = await returnPlan({ userId, weekStart, comment: c }); if (r?.error) toast.error(r.error); else { toast.success(t(locale, "plan.returnedRevision")); router.refresh(); } }); }} disabled={pending}>
