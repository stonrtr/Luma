...
export function CallPointsPanel({ members }: { members: Member[] }) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      {members.length === 0 && <p className="text-sm text-muted-foreground">{tr("cp.noOthers")}</p>}
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        {openCount > 0 && <span className="ml-auto rounded-full bg-accent px-2 py-0.5 text-xs font-semibold text-accent-foreground">{openCount}</span>}
      </div>
      ...
      <ul className="mb-2 space-y-1">
        {member.points.map((p) => (
          <li key={p.id} className="group flex items-center gap-2 text-sm">
            <button
              ...
              className={cn("flex size-4 shrink-0 items-center justify-center rounded border transition-colors", p.done ? "border-primary bg-primary text-primary-foreground" : "border-muted-foreground/40 hover:border-primary")}
              title={p.done ? tr("cp.removed") : tr("cp.mark")}
            >
        ...
        {member.points.length === 0 && <li className="text-xs text-muted-foreground">{tr("cp.noPoints")}</li>}
      </ul>

      <div className="flex gap-2">
        <Input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} placeholder={tr("cp.discussPh")} className="h-8" />
