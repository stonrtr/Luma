  ...
  return (
    <div>
      <h3 className="mb-2 flex items-center gap-1.5 text-sm font-semibold">
        <Target className="size-4 text-primary" /> Цілі місяця
      </h3>
      <ul className="space-y-1.5">
        ...
        {goals.length === 0 && <li className="text-sm text-muted-foreground">Цілей поки немає.</li>}
      </ul>
      {canManage && (
        <div className="mt-2 flex gap-2">
          <Input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} placeholder="Нова ціль на місяць…" className="h-8" />
