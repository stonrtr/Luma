...
export function UserMenu({
  ...
  const item = "flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-muted";
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        <div className="pt-1">
          <Link href="/settings" className={item}><Settings className="size-4 text-muted-foreground" />{t(locale, "settings.title")}</Link>
          <button onClick={() => start(() => signOutAction())} className={item}>
