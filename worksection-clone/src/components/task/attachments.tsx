  ...
  return (
    <div>
      <h3 className="mb-3 flex items-center gap-1.5 text-sm font-semibold">
        <Paperclip className="size-4" /> Вложения ({items.length})
      </h3>
      ...
      <ul className="space-y-1.5">
        {items.map((a) => (
          <li key={a.id} className="group flex items-center gap-2 rounded-md border px-2 py-1.5 text-sm">
            ...
            <a href={a.url} target="_blank" rel="noreferrer" className="min-w-0 flex-1 truncate hover:text-primary">
              {a.fileName}
            </a>
      ...
      <div
        ...
        className={`mt-2 flex cursor-pointer flex-col items-center gap-1 rounded-lg border border-dashed py-4 text-xs text-muted-foreground transition-colors hover:border-primary/40 hover:text-foreground ${
          dragging ? "border-primary/60 bg-accent/50" : ""
        }`}
      >
        <Upload className="size-4" />
        {pending ? "Загрузка…" : "Перетащите файл или нажмите (до 10 МБ)"}
      </div>
      <input
        ref={inputRef}
        type="file"
