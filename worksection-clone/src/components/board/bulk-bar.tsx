...
export function BulkBar() {
  ...
  function apply(status: TaskStatus) {
    start(async () => {
      ...
      toast.success(`Оновлено: ${ids.length}`);
      sel!.clear();
      router.refresh();
    });
  }
  ...
  return (
    <div className="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full border bg-card px-3 py-2 shadow-lg">
      <span className="px-1 text-sm font-medium">Вибрано: {ids.length}</span>
      <button
        onClick={() => apply("DONE")}
        disabled={pending}
        className="flex items-center gap-1 rounded-full bg-emerald-500 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-emerald-600 disabled:opacity-50"
      >
        <CheckCircle2 className="size-4" /> Закрити
      </button>
      <Popover>
        <PopoverTrigger className="flex items-center gap-1 rounded-full border px-3 py-1.5 text-sm font-medium hover:bg-muted">
          <ArrowRightLeft className="size-4" /> Перемістити
        </PopoverTrigger>
      ...
      <button onClick={() => sel.clear()} className="rounded-full p-1.5 text-muted-foreground hover:bg-muted" title="Зняти виділення">
        <X className="size-4" />
      </button>
    </div>
  );
