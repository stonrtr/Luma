...
describe("parseStructuredSummary", () => {
  ...
  it("прошедшая дата без года остаётся в текущем году (задача сразу просрочена)", () => {
    const r = parseStructuredSummary("Задачі\nЩось. До 01.02. 4")!; // 01.02 уже прошло — НЕ переносим
    expect(r.tasks[0].dueLabel).toBe(`01.02.${new Date().getFullYear()}`);
  });
...
describe("однорядкова задача без секції «Задачі»", () => {
  it("рядок з усіма трьома параметрами стає задачею", () => {
    const r = parseStructuredSummary("Зробити самарі №2. До 24.07. 30 хв. 2")!;
    expect(r).not.toBeNull();
    expect(r.tasks).toHaveLength(1);
    expect(r.tasks[0].title).toBe("Зробити самарі №2");
    expect(r.tasks[0].priority).toBe(2);
    expect(r.tasks[0].plannedMinutes).toBe(30);
    expect(r.tasks[0].dueLabel).toMatch(/^24\.07\./);
  });
  it("рядок без якогось параметра — не задача (null, піде в ІІ/вільний текст)", () => {
    expect(parseStructuredSummary("Просто якийсь текст без параметрів")).toBeNull();
    expect(parseStructuredSummary("Назва задачі. До 24.07")).toBeNull();
  });
});
