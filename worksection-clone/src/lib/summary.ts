...
// Без року → завжди ПОТОЧНИЙ рік: минула дата лишається минулою (задача одразу
// прострочена) — рішення користувача, перенесення на наступний рік немає.
export function parseDueDate(day: number, month: number, year?: number): Date | null {
  ...
  const d = new Date(y, month - 1, day, 19, 0, 0, 0);
...
export function parseTaskLine(line: string): StructuredTask | null {
  const raw = line.trim();
  if (!raw) return null;
...
export function parseStructuredSummary(summary: string): StructuredSummary | null {
  ...
  if (!hasSection) {
    // Без секцій: рядок, у якому Є ВСІ ТРИ параметри (пріоритет + дедлайн + час),
    // однозначно є задачею — слово «Задачі» писати не потрібно.
    const tasks: StructuredTask[] = [];
    for (const line of lines) {
      const item = line.replace(/^\s*(?:[-*•·▪◦]|\d+[.)])\s+/u, "").trim();
      if (!item) continue;
      const t = parseTaskLine(item);
      if (t && t.priority != null && t.dueISO != null && t.plannedMinutes != null) tasks.push(t);
    }
    return tasks.length > 0 ? { ideas: [], tasks } : null;
  }
