...
// Отправить сообщение пользователю приложения (если он привязал Telegram). extra — доп. поля (reply_markup)
export async function sendTelegramToUser(userId: string, text: string, extra?: Record<string, unknown>): Promise<void> {
  if (!TOKEN) return;
  const acc = await db.telegramAccount.findUnique({ where: { userId }, select: { chatId: true } });
  if (!acc) return;
  await sendTelegram(acc.chatId, text, extra);
}
