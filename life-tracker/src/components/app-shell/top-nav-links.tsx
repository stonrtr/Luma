...
import { LayoutDashboard, ClipboardList, LayoutGrid, FolderKanban } from "lucide-react";
import { t } from "@/lib/i18n";
import { cn } from "@/lib/utils";

export function TopNavLinks({ locale, isAdmin: _isAdmin }: { locale: string; isAdmin: boolean }) {
  const pathname = usePathname();
  const items = [
    { href: "/", key: "nav.myspace", icon: LayoutDashboard },
    { href: "/projects", key: "nav.projects", icon: FolderKanban },
    // Огляд — зведення прогресу по всіх векторах
    { href: "/team", key: "nav.team", icon: LayoutGrid },
