        ...
        <div className="space-y-4">
          ...
          {recurring && (
            <>
              ...
              {frequency === "MONTHLY" && (
                <div className="space-y-1.5">
                  <Label htmlFor="rec-start">{t(locale, "dlg.startTime")}</Label>
                  <Input id="rec-start" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                  <p className="text-[11px] text-muted-foreground">{t(locale, "dlg.monthlyHint")} {Number(startDate.split("-")[2])}</p>
                </div>
              )}
            </>
          )}
