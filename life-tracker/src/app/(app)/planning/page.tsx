...
import { Gauge, ListChecks, ChevronDown } from "lucide-react";
...
function KpiMonthDetails({ g, showYear = true, locale }: { g: KpiMonthGroup; showYear?: boolean; locale: string }) {
  return (
        ...
        <div className="rounded-xl border bg-card p-5">
          ...
          {kpiArchive.length === 0 ? (
            ...
            <div className="space-y-1.5">
              ...
              {kpiCurrentMonths.map((g) => <KpiMonthDetails key={`${g.year}-${g.month}`} g={g} showYear={false} locale={viewer.locale} />)}
              ...
              {kpiPastByYear.map((y) => (
                <details key={y.year} className="group rounded-lg border bg-muted/20 px-3 py-2">
                  ...
                  <div className="mt-2 space-y-1.5">
                    {y.months.map((g) => <KpiMonthDetails key={`${g.year}-${g.month}`} g={g} showYear={false} locale={viewer.locale} />)}
                  </div>
                </details>
              ))}
            </div>
