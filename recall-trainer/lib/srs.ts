// Spaced repetition (FSRS-подобный) — ТЗ §27. Перенесён из luma-trainer.
// Три оценки: again = «Не помню», hard = «Тяжело», easy = «Помню/Легко».

export type Rating = "again" | "hard" | "easy";

const DAY_MS = 24 * 60 * 60 * 1000;
const MINUTE_DAYS = 1 / (24 * 60);
const AGAIN_DELAY_DAYS = 10 * MINUTE_DAYS; // ~10 минут
...
export function review(
  ...
  if (rating === "again") {
    ...
    const diffFactor = 1 - (difficulty - 1) / 18;
